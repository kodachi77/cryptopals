...
break_repeating_xor_key( char** out_plaintext, char** out_key, const char* src, size_t src_len )
{
    ...
    char   chunks[N_CHUNKS][MAX_KEY_LENGTH];
    double hamming_dist[MAX_KEY_LENGTH];

    char key[MAX_KEY_LENGTH + 1];

    size_t i, j, k, n, key_size;
    ...
    for( key_size = 2; key_size < 42; ++key_size )
    {
        ...
        {
            for( j = i + 1; j < N_CHUNKS; j++ ) { hamming_dist[key_size] += hamming_distance( chunks[i], chunks[j], key_size ); }
        }
...
main( void )
{
    ...
    char * base64_data, *binary_data, *plaintext, *key;
    size_t b64len;
    int    ret;
